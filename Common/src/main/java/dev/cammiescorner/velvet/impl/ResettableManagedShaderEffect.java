...
import dev.cammiescorner.velvet.api.util.ShaderPrograms;
import dev.cammiescorner.velvet.mixin.client.AccessiblePassesShaderEffect;
...
public final class ResettableManagedShaderEffect extends ResettableManagedShaderBase<PostChain> implements ManagedShaderEffect {
	...
	private final Map<String, RenderTargetWrapper> managedTargets;
	...
	public void setup(int windowWidth, int windowHeight) {
		...
		for(RenderTargetWrapper buf : this.managedTargets.values()) {
	...
	@Override
	public ManagedRenderTarget getTarget(String name) {
		return this.managedTargets.computeIfAbsent(name, n -> {
			RenderTargetWrapper ret = new RenderTargetWrapper(n);
			if(this.shader != null) {
	...
	protected void logInitError(IOException e) {
		Velvet.LOGGER.error("Could not create screen shader {}", this.getLocation(), e);
	}
