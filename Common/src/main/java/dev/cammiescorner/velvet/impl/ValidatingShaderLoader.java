...
import dev.cammiescorner.velvet.api.util.ShaderLinkException;
import dev.cammiescorner.velvet.api.util.ShaderLoader;
...
public final class ValidatingShaderLoader implements ShaderLoader {
	 ...
	 */
	public int loadShader(ResourceProvider resourceManager, @Nullable ResourceLocation vertexLocation, @Nullable ResourceLocation fragmentLocation) throws IOException {
		...
		if(vertexLocation != null) {
			...
			if(!log.isEmpty()) {
				Velvet.LOGGER.error("Could not compile vertex shader {}: {}", vertexLocation, log);
			}
		}
		...
		if(fragmentLocation != null) {
			...
			if(!log.isEmpty()) {
				Velvet.LOGGER.error("Could not compile fragment shader {}: {}", fragmentLocation, log);
			}
		}
		...
		if(glGetProgrami(programId, GL_VALIDATE_STATUS) == 0) {
			Velvet.LOGGER.warn("Warning validating Shader code: {}", glGetProgramInfoLog(programId, 1024));
		}
	 ...
	 */
	private String fromFile(ResourceProvider resourceManager, ResourceLocation fileLocation) throws IOException {
		StringBuilder source = new StringBuilder();

		try(InputStream in = resourceManager.getResourceOrThrow(fileLocation).open();
